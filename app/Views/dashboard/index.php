    ...
    <div class="container-fluid px-4">
      ...
      <div class="flex row">
        <div class="col-md-3">
          ...
          <div class="card card-danger mt-4">
            <div class="card-header">
              <h3 class="card-title">Status Pemakaian</h3>
            </div>
            <div class="card-body">
        ...
        <div class="col-md-9">
          <div class="card">
            <div class="card-header">
              <h3 class="card-title">Data Pemakai Bahan Kimia</h3>
            </div>
            <!-- /.card-header -->
            <div class="card-body">
              <table id="example1" class="table table-bordered table-striped">
                <thead>
                  <tr>
                    <th>Nama</th>
                    <th width="120">Alkohol</th>
                    <th width="120">Spirtus</th>
                    <th width="120">Akuades</th>
                    <th>Waktu</th>
...
<script>
  ...
  // Get Data
  firebase.database().ref('pemakai/').orderByChild('waktu').on('value', function(snapshot) {
    var value = snapshot.val();
    var htmls = [];
    var hAlko = 0;
    var hSpir = 0;
    ...
    $.each(value, function(index, value) {
      if (value) {
        if (data3 != 'admin') {
          if (value.npm == npm) {
            hAlko = value.alkohol + hAlko;
            hSpir = value.spirtus + hSpir;
            hAqua = value.aquades + hAqua;
            htmls.push('<tr>\
                <td>' + value.nama + '</td>\
                <td>' + value.alkohol + '</td>\
                <td>' + value.spirtus + '</td>\
    ...
    $(function() {
      ...
      var donutData = {
        labels: ["Alkohol", "Spirtus", "Akuades"],
        datasets: [{
          data: [hAlko, hSpir, hAqua],
          backgroundColor: [
            "#f56954",
            "#00a65a",
            "#f39c12",
  ...
  function writeUserData(name, npm, alkohol, spirtus, aquades) {
    firebase.database().ref('pemakai/' + datawaktu).set({
      nama: name,
      npm: npm,
      alkohol: alkohol,
      spirtus: spirtus,
  ...
  if (status == 'admin') {
    ...
    $('.deleteStudent').on('click', function() {
      ...
      firebase.database().ref('pemakai/' + id).remove();
